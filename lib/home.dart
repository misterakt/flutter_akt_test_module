...
class _HomeState extends State<Home> {
  ...
  _connectToChat(QBUser user) async {
    try {
      await QB.chat.connect(user.id, "01741789858");
    } catch (e) {
      print("error");
      print(e);
    }
  ...
  _createChatOneByOne() async {
    try {
      QBDialog createdDialog = await QB.chat.createDialog(
          [134016141, 134437503], "Private Chat - 2",
  ...
  _getHistory() async {
    try {
      List<QBMessage> messages = await QB.chat
          .getDialogMessages("625da73f32eaaf0022ce65b3", markAsRead: true);
